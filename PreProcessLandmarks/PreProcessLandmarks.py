...
class PreProcessLandmarksWidget(ScriptedLoadableModuleWidget):
  ...
  def OnHomogenizeWidthButtonClicked(self):
    self.logic.PatientModel.HomogenizeVertebralInterlandmarkWidth(self.HomogenizationFactorSlider.value)
...
class RepairLandmarksLogic(ScriptedLoadableModuleLogic):
  class PatientTransverseProcesses:
    ...
    def HomogenizeVertebralInterlandmarkWidth(self, HomogenizationFactor):
      ...
      # Get inter-landmark widths and stats
      WidthsArray = []
      for pi in range(self.LeftSide.MarkupsNode.GetNumberOfFiducials()):
        LeftCoords = self.LeftSide.MarkupsNode.GetMarkupPointVector(pi,0)
        RightCoords = self.RightSide.MarkupsNode.GetMarkupPointVector(pi,0)
        LeftRightVector = [RightCoords[dim] - LeftCoords[dim] for dim in range(3)]
        CurrentWidth = np.linalg.norm(LeftRightVector)
        WidthsArray.append(CurrentWidth)
        
      AverageWidth = np.mean(WidthsArray)
      
      # Generate new landmarks, pair-wise, closer or further apart based on average (global) and/or local widths compared to current width
      NewLeftSide = slicer.vtkMRMLMarkupsFiducialNode()
      NewLeftSide.SetName(self.LeftSide.MarkupsNode.GetName())
      NewRightSide = slicer.vtkMRMLMarkupsFiducialNode()
      NewRightSide.SetName(self.RightSide.MarkupsNode.GetName())
      
      # Global information re-spacing
      for pi in range(self.LeftSide.MarkupsNode.GetNumberOfFiducials()):
        LeftCoords = self.LeftSide.MarkupsNode.GetMarkupPointVector(pi,0)
        RightCoords = self.RightSide.MarkupsNode.GetMarkupPointVector(pi,0)
        LeftRightVector = [RightCoords[dim] - LeftCoords[dim] for dim in range(3)]
        CurrentWidth = np.linalg.norm(LeftRightVector)
        LeftRightUnitVector = [LeftRightVector[dim]/CurrentWidth for dim in range(3)]
        PointDisplacementLength = (CurrentWidth - AverageWidth)/2.0
        
        NewLeftCoords = [LeftCoords[dim] + (LeftRightUnitVector[dim] * PointDisplacementLength) for dim in range(3)]
        NewRightCoords = [RightCoords[dim] - (LeftRightUnitVector[dim] * PointDisplacementLength) for dim in range(3)]
  
        self.LeftSide.MarkupsNode.SetNthFiducialPositionFromArray(pi, NewLeftCoords)
        self.RightSide.MarkupsNode.SetNthFiducialPositionFromArray(pi, NewRightCoords)
